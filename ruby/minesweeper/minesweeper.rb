...
class Board
  ALLOWED_CHARS = ['|', '-', '+', '*', ' '].freeze

  def self.transform(input)
    raise ArgumentError unless input.min.length == input.max.length && input.join.chars.all? do |x|
                                 ALLOWED_CHARS.include?(x)
                               end && input.first == input.last

    @array_of_rows = input.map(&:chars)
    @frame = ['|', '-', '+']
    @da_bomb = '*'
    @empty = ' '
    @array_of_rows.each_with_index do |row, row_index|
      row.each_with_index do |elem, elem_index|
        next unless elem == @da_bomb
